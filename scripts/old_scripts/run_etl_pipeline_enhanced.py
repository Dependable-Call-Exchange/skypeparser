...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the ETL pipeline on Skype export data"
    )
    ...
    user_group.add_argument(
        "-u",
        "--user",
        dest="user_display_name",
        help="Your display name in the Skype conversations",
    )
    ...
    db_group.add_argument(
        "-W", "--password", dest="db_password", help="Database password"
    )
    db_group.add_argument(
        "--create-tables",
        dest="create_tables",
    ...
    output_group.add_argument(
        "--output-dir", dest="output_dir", help="Directory to store output files"
    )
    ...
    checkpoint_group.add_argument(
        "--checkpoint", dest="checkpoint_id", help="Checkpoint ID to resume from"
    )
    ...
    attachment_group.add_argument(
        ...
        action="store_true",
        help="Download attachments from messages",
    )
    attachment_group.add_argument(
        "--attachments-dir",
    ...
    parser.add_argument(
        "--select-json",
        dest="select_json",
        help="Path to JSON file with conversation IDs to select",
    )
...
def get_etl_context(args, config):
    ...
    context = ETLContext(
        ...
        or config.get("output", {}).get("directory", "output"),
...
def download_attachments(pipeline_result, args):
    ...
    handler = AttachmentHandler(
        output_dir=attachments_dir, download_thumbnails=not args.no_thumbnails
    )
...
def print_summary(pipeline_result):
    ...
    logger.info(
        f"  Extract Phase: {'Success' if extract_success else 'Failed'} ({extract_time:.2f}s)"
    )
    ...
    logger.info(
        f"  Transform Phase: {'Success' if transform_success else 'Failed'} ({transform_time:.2f}s)"
    )
    ...
    logger.info(
        f"  Load Phase: {'Success' if load_success else 'Failed'} ({load_time:.2f}s)"
    )
...
def main():
    ...
    try:
        ...
        # Create the ETL pipeline with context
        pipeline = ETLPipeline(db_config=context.db_config, context=context)
        ...
        if args.resume or args.checkpoint_id:
            ...
            result = pipeline.run_pipeline(
                resume_from_checkpoint=True, checkpoint_id=checkpoint_id
            )
        else:
            # Run from scratch
            if not args.file_path:
                logger.error(
                    "File path is required when not resuming from a checkpoint"
                )
                sys.exit(1)

            logger.info(f"Processing file: {args.file_path}")
            result = pipeline.run_pipeline(
                file_path=args.file_path, user_display_name=args.user_display_name
            )
