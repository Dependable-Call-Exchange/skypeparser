...
from src.core_utils.di import get_service, get_service_provider
...
class ETLPipeline:
    ...
    def __init__(
        ...
        if context:
            ...
            if file_path and not hasattr(self.context, 'file_path'):
    ...
    def run_pipeline(
        ...
        # Set export date if not already set
        if not self.context.export_date:
            self.context.export_date = datetime.now().isoformat()
    ...
    def save_checkpoint(self, checkpoint_dir=None):
        ...
        try:
            ...
            # Create a serializable representation of the context with safe attribute access
            checkpoint_data = {
                "task_id": self.context.task_id,
                "start_time": self._format_datetime(getattr(self.context, 'start_time', None)),
                "current_phase": getattr(self.context, 'current_phase', None),
                "db_config": getattr(self.context, 'db_config', {}),
                "output_dir": getattr(self.context, 'output_dir', ''),
                "memory_limit_mb": getattr(self.context, 'memory_limit_mb', 1024),
                "parallel_processing": getattr(self.context, 'parallel_processing', True),
                "chunk_size": getattr(self.context, 'chunk_size', 1000),
                "batch_size": getattr(self.context, 'batch_size', 1000),
                "phase_statuses": getattr(self.context, 'phase_statuses', {}),
                "phase_results": getattr(self.context, 'phase_results', {}),
                "checkpoints": getattr(self.context, 'checkpoints', {}),
                "errors": getattr(self.context, 'errors', []),
                "export_id": getattr(self.context, 'export_id', None),
                "file_path": getattr(self.context, 'file_path', None),
            }

            # Add metrics data if available
            metrics = getattr(self.context, 'metrics', {})
            if metrics:
                checkpoint_data["metrics"] = {
                    "start_time": self._format_datetime(metrics.get("start_time")),
                    ...
                    "duration": {
                        ...
                        for phase, times in metrics.get("duration", {}).items()
                    },
                }

            # Add all serializable attributes from the context
            if hasattr(self.context, 'SERIALIZABLE_ATTRIBUTES'):
            ...
            # Continue with pipeline execution instead of raising the exception
            return None
    ...
    def _run_extract_phase(
        ...
        try:
            ...
            if file_path:
                self.context.file_path = file_path
                logger.info(f"Set file path in context: {file_path}")
            ...
            if self.context.output_dir:
                try:
                    ...
                    logger.error(f"Error saving checkpoint after extraction: {e}")
        ...
        try:
            ...
            if self.context.output_dir:
                try:
                    ...
                    logger.error(f"Error saving checkpoint after transformation: {e}")
                    # Continue with the pipeline execution
    ...
    def _run_load_phase(
        ...
        try:
            ...
            # Set file source in the context if not already set
            if file_source and not self.context.file_path:
                self.context.file_path = file_source
                logger.info(f"Set file path in context for loading: {file_source}")
            ...
            try:
                ...
                if self.context.output_dir:
                    try:
                        ...
                        logger.error(f"Error saving checkpoint after loading: {e}")
                        # Continue with the pipeline execution
