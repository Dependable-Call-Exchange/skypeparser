...
from src.core_utils.service_registry import (
...
class ImprovedTestableETLPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            output_dir: Output directory for ETL results (defaults to a temporary directory)
        """
        ...
        self.context: ETLContext = ETLContext(db_config=db_config, output_dir=output_dir)
    ...
    def load(
        ...
        # Also pass file_path as file_source parameter
        ...
        # The loader returns a dict with counts, but the expected return is an int
        # Return the total number of records as the ID
        total_records = sum(load_result.values()) if isinstance(load_result, dict) else 0
...
def create_testable_etl_pipeline(
    ...
    pipeline = ImprovedTestableETLPipeline(
        db_config=db_config,
        file_handler=file_handler or default_file_handler,
        validation_service=validation_service or default_validation_service,
        db_connection=db_connection or MockDatabase(),
        content_extractor=content_extractor or MockContentExtractor(),
        structured_data_extractor=structured_data_extractor or MockStructuredDataExtractor(),
        message_handler_factory=message_handler_factory or MockMessageHandlerFactory(),
...
class TestableETLPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            validate_file_exists_func: Function to validate file existence
            validate_json_file_func: Function to validate JSON files
            validate_user_display_name_func: Function to validate user display names
        ...
        # Handle case where output_dir is passed as first parameter (for backward compatibility)
        if db_config is None and output_dir is None and isinstance(db_connection, str):
            output_dir = db_connection
            db_connection = None

        # If db_config is not provided, create a default one
        if db_config is None:
            db_config = {"dbname": "test_db", "user": "test_user", "password": "test_password", "host": "localhost"}
        ...
        if not use_di:
            ...
            from src.core_utils.di import ServiceProvider
