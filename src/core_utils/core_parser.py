...
from src.core_utils.config import get_message_type_description, load_config

# Import centralized dependency handling
from src.core_utils.dependencies import BEAUTIFULSOUP_AVAILABLE as BEAUTIFULSOUP
from src.core_utils.dependencies import BS_PARSER, BeautifulSoup

# Import message type handlers
from src.messages.message_type_handlers import extract_structured_data, get_handler_for_message_type

# Import content extraction module
from src.content_extractor import ContentExtractor, extract_content_data, format_content_with_markup

# Import custom exceptions
from src.exceptions import ContentParsingError, DataExtractionError, InvalidInputError, TimestampParsingError
...
def parse_skype_data(
    ...
    try:
        ...
        # Convert dict of conversations to list for compatibility
        conversations_list = []
...
def _process_conversations(
    ...
    for i in range(len(raw_data["conversations"])):
        try:
            ...
            # Process messages - handle both newer "MessageList" format and older "messages" format
...
def _process_messages(
    ...
    """
    # Support both newer "MessageList" format and older "messages" format
    messages = []
    if "MessageList" in conversation and isinstance(conversation["MessageList"], list):
        messages = conversation["MessageList"]
        logger.debug(f"Found MessageList with {len(messages)} messages")
    elif "messages" in conversation and isinstance(conversation["messages"], list):
        messages = conversation["messages"]
        logger.debug(f"Found messages with {len(messages)} messages")
    else:
        logger.warning(f"No messages found in conversation {conversation.get('id')}")
        return []
...
def _process_message_content(message_data, message_type):
    ...
    Returns:
        Tuple of (content, structured_data)
    """
    ...
    try:
        handler_func = get_handler_for_message_type(message_type)
        if handler_func:
            # Call the function directly, since get_handler_for_message_type
            # returns the extract_structured_data function, not an object
            handler_data = handler_func(message_data)
            if handler_data:
