...
class TextMessageHandler(BaseMessageHandler):
    ...
    def extract_structured_data(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'call' in message_type and '<duration>' in content:
            ...
            participants = []
            participant_matches = re.finditer(r'<participant id="(.*?)" name="(.*?)"></participant>', content)
        ...
        try:
            ...
            # Contacts
                ...
                if '<contact name=' in content:
                    ...
                    contacts = []
                    contact_matches = re.finditer(r'<contact name="(.*?)" phone="(.*?)" email="(.*?)"></contact>', content)
            ...
            # Media Video/Image
                ...
                if '<uriobject' in content:
                    ...
                    # Add media type
                    data['media_type'] = 'video' if 'media_video' in message_type else 'image'
            ...
            # Media Album
                ...
                if '<img' in content:
                    # Extract album items
                    img_matches = re.finditer(r'<img src="(.*?)" thumbnail="(.*?)" width="(.*?)" height="(.*?)" />', content)
                    album_items = []
            ...
            # Translation
                ...
                if '<translation' in content:
                    ...
                    translation_match = re.search(r'<translation[^>]*>(.*?)</translation>', content)
            ...
            # Thread Activity
                ...
                if '/' in message_type:
                    ...
                    if activity_type_raw.lower() == 'addmember':
                        ...
                        activity_type = activity_type_raw[0].upper() + activity_type_raw[1:]
                else:
                ...
                if 'addmember' in message_type.lower():
                    ...
                    if '<member' in content:
                        ...
                        member_matches = re.finditer(r'<member id="(.*?)" name="(.*?)"></member>', content)
                        members = []
                        ...
                        # Extract initiator
                        initiator_match = re.search(r'<initiator name="(.*?)"></initiator>', content)
                        if initiator_match:
    ...
    def extract_structured_data(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if '<pollquestion>' in content:
                ...
                poll_question_match = re.search(r'<pollquestion>(.*?)</pollquestion>', content)
            ...
            data['poll_question'] = poll_title  # Set poll_question to match poll_title for compatibility
