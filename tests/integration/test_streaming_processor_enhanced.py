...
def create_large_streaming_file(file_path, conversation_count=5, messages_per_conversation=200):
...
class TestStreamingProcessorEnhanced(unittest.TestCase):
    ...
    def test_basic_streaming_processing(self):
        ...
        result = processor.process_file(
            ...
            user_display_name="Streaming Test User"
        )
        ...
        self.assertEqual(result['processed_count'], 300, "Expected 300 messages to be processed")
        ...
        with test_db_connection(self.db_config) as conn:
            with conn.cursor() as cursor:
                ...
                self.assertEqual(conversation_count, 3, "Expected 3 conversations in database")
                ...
                self.assertEqual(message_count, 300, "Expected 300 messages in database")
    ...
    def test_checkpoint_creation(self):
        ...
        result = processor.process_file(
            file_path=self.test_file,
            user_display_name="Checkpoint Test User"
        )
        ...
        latest_checkpoint = os.path.join(self.checkpoint_dir, sorted(checkpoint_files)[-1])
        with open(latest_checkpoint, 'r') as f:
            checkpoint_data = json.load(f)

        # Verify checkpoint data
        self.assertIn('export_id', checkpoint_data, "Checkpoint missing export_id")
        self.assertIn('processed_count', checkpoint_data, "Checkpoint missing processed_count")
        self.assertEqual(checkpoint_data['processed_count'], 300, "Checkpoint shows incorrect processed count")
    ...
    def test_interruption_and_resume(self):
        ...
        with patch.object(processor, '_process_message', side_effect=interrupted_process):
            try:
                # This should be interrupted
                processor.process_file(
                    file_path=self.test_file,
                    user_display_name="Interrupted Test User"
                )
        ...
        checkpoint_files = [f for f in os.listdir(self.checkpoint_dir) if f.startswith('checkpoint_')]
        self.assertGreater(len(checkpoint_files), 0, "No checkpoint files were created during interruption")
        ...
        self.assertIn('export_id', checkpoint_data, "Checkpoint missing export_id")
        self.assertIn('processed_count', checkpoint_data, "Checkpoint missing processed_count")
        self.assertGreaterEqual(checkpoint_data['processed_count'], 75,
                              "Checkpoint processed count too low")
        self.assertLessEqual(checkpoint_data['processed_count'], 100,
        ...
        self.assertEqual(resume_result['processed_count'], 300,
        ...
        with test_db_connection(self.db_config) as conn:
            with conn.cursor() as cursor:
                ...
                """, (checkpoint_data['export_id'],))
                message_count = cursor.fetchone()[0]
                self.assertEqual(message_count, 300,
                               "Expected 300 messages in database after resume")
    ...
    def test_multiple_interruptions_and_resumptions(self):
        ...
        with patch.object(processor, '_process_message',
                        side_effect=lambda m, c, **kw: self._interrupt_after(
                            processor._process_message, m, c, 100, **kw)):
            try:
                processor.process_file(
                    file_path=self.test_file,
                    user_display_name="Multi-Interrupted Test"
                )
        ...
        checkpoint_files = sorted([f for f in os.listdir(self.checkpoint_dir) if f.startswith('checkpoint_')])
        ...
        with patch.object(processor2, '_process_message',
                        side_effect=lambda m, c, **kw: self._interrupt_after(
                            processor2._process_message, m, c, 200, **kw)):
            try:
                processor2.process_file(
                    file_path=self.test_file,
                    resume=True
                )
        ...
        # Complete processing
        final_result = processor3.process_file(
            file_path=self.test_file,
            resume=True
        )
        ...
        # Verify total processed messages
        self.assertEqual(final_result['processed_count'], 300,
    ...
    def test_checkpoint_file_location(self):
        ...
        result = processor.process_file(
            file_path=self.test_file,
            user_display_name="Custom Checkpoint Test"
        )
        ...
        checkpoint_files = [f for f in os.listdir(custom_checkpoint_dir) if f.startswith('checkpoint_')]
    ...
    def test_automatic_checkpoint_cleanup(self):
        ...
        result = processor.process_file(
            file_path=self.test_file,
            user_display_name="Checkpoint Cleanup Test"
        )
        ...
        checkpoint_files = [f for f in os.listdir(self.checkpoint_dir) if f.startswith('checkpoint_')]
        self.assertLessEqual(len(checkpoint_files), 2,
                          f"Too many checkpoint files: {len(checkpoint_files)}, expected at most 2")

    def _interrupt_after(self, original_method, message, conversation_id, threshold, **kwargs):
        ...
        if processed_count >= threshold:
            raise SimulatedInterruption(f"Simulated interruption after {threshold} messages")
