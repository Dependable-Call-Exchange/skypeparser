...
from tests.factories import MockBuilderFactory, SkypeDataFactory
...
class TestETLPipelineIntegrationEnhanced(unittest.TestCase):
    ...
    def test_dependency_injected_pipeline(self):
        ...
        with test_db_connection() as conn:
            ...
            result = pipeline.run_pipeline(
                file_path="non_existent_file.json", user_display_name="DI Test User"
            )
            ...
            mock_functions["validate_file_exists"].assert_called_with(
    ...
    def test_complex_data_processing(self):
        ...
        result = self.pipeline.run_pipeline(
            file_path=self.complex_file, user_display_name="Complex Data Test"
        )
        ...
        with self.pipeline.conn.cursor() as cur:
            ...
            self.assertEqual(
                conversation_count, len(self.complex_data["conversations"])
            )
    ...
    def test_pipeline_with_logging(self):
        ...
        with test_db_connection() as conn:
            pipeline = TestableETLPipeline(
                output_dir=self.test_dir, db_connection=conn, logger=logger
            )

            # Run the pipeline
            result = pipeline.run_pipeline(
                file_path=self.basic_file, user_display_name="Logging Test User"
            )
            ...
            self.assertGreater(
                len(log_capture), 10, "Expected at least 10 log messages"
            )
        ...
        with test_db_connection() as conn:
            ...
            with self.assertRaises(ValueError) as context:
                pipeline.run_pipeline(
                    file_path=self.basic_file, user_display_name="Error Test User"
                )
    ...
    def test_pipeline_events(self):
        ...
        with test_db_connection() as conn:
            ...
            # Run the pipeline
            result = pipeline.run_pipeline(
                file_path=self.basic_file, user_display_name="Events Test User"
            )
    ...
    def test_incremental_processing(self):
        ...
        result1 = self.pipeline.run_pipeline(
            file_path=self.basic_file, user_display_name="Incremental Test User"
        )
        ...
        with self.pipeline.conn.cursor() as cur:
            ...
            expected_message_count = sum(
                len(conv.get("MessageList", []))
                for conv in self.basic_data["conversations"]
            ) + sum(
                len(conv.get("MessageList", []))
                for conv in self.complex_data["conversations"]
            )
