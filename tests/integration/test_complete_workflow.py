...
class TestCompleteWorkflow(unittest.TestCase):
    ...
    def test_cli_workflow(self):
        ...
        etl_result = pipeline.run_pipeline(
            ...
            user_display_name="CLI Test User"
        )

        # Verify ETL success
        self.assertTrue(etl_result['success'], f"ETL pipeline failed: {etl_result.get('error', 'Unknown error')}")
        ...
        self.assertIn('conversation_count', report_result, "Report result missing conversation count")
        self.assertIn('message_count', report_result, "Report result missing message count")
        self.assertGreater(report_result['message_count'], 0, "Report shows no messages")
        ...
        with test_db_connection(self.db_config) as conn:
            with conn.cursor() as cursor:
                ...
                self.assertGreater(conversation_count, 0, "No conversations found in database")
    ...
    def test_api_workflow(self):
        ...
        self.assertEqual(upload_response.status_code, 200, f"Upload failed: {upload_response.text}")
        ...
        while status in ('pending', 'processing') and retry_count < max_retries:
            ...
            self.assertEqual(status_response.status_code, 200, f"Status check failed: {status_response.text}")
            status_data = status_response.json()
            status = status_data.get('status', 'unknown')
            retry_count += 1

        # Verify processing completed successfully
        self.assertEqual(status, 'completed', f"Processing failed or timed out. Final status: {status}")
        self.assertIn('export_id', status_data, "Export ID not found in status response")
        export_id = status_data['export_id']

        # Step 3: Get analysis results
        analysis_response = client.get(f"/api/analysis/{export_id}")
        self.assertEqual(analysis_response.status_code, 200, f"Analysis failed: {analysis_response.text}")
        analysis_data = analysis_response.json()

        # Verify analysis data
        self.assertIn('message_count', analysis_data, "Analysis missing message count")
        self.assertIn('conversation_count', analysis_data, "Analysis missing conversation count")
        self.assertGreater(analysis_data['message_count'], 0, "Analysis shows no messages")
        ...
        self.assertIn(b'<html', report_response.content, "Response does not contain HTML content")
