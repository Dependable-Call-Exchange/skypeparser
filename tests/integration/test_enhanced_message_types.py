...
class MockETLComponents:
    ...
    def register_di_components(self):
        ...
        service_provider.register_singleton(DatabaseConnectionProtocol, self.db_connection)
...
class TestEnhancedMessageTypes(unittest.TestCase):
    ...
    def test_poll_message_extraction(self):
        ...
        self.assertEqual(data['poll_title'], "What's your favorite programming language?")
    ...
    def test_scheduled_call_extraction(self):
        ...
        self.assertEqual(call_data['description'], 'Discuss project progress and next steps')

        # Check meeting link
        self.assertTrue(call_data['meeting_link'].startswith('https://teams.microsoft.com/'))
    ...
    def test_attachment_processing(self, mock_download):
        ...
        # Initialize attachment handler
        handler = AttachmentHandler(storage_dir=os.path.join(self.temp_dir, 'attachments'))
    ...
    def test_etl_pipeline_with_enhanced_types(self):
        ...
        call_messages = [msg for msg in messages if msg.get('message_type') == 'RichText/ScheduledCallInvite']
        self.assertEqual(len(call_messages), 1)
        call_message = call_messages[0]
        self.assertIn('scheduled_call', call_message)

        # Find media message
        media_messages = [msg for msg in messages if msg.get('message_type') == 'RichText/Media_GenericFile']
