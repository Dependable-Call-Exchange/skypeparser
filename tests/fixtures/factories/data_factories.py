...
# Expected data factories for API responses and transformed data
class ExpectedApiResponseFactory(factory.Factory):
    """Factory for generating expected API response data."""
    ...
    data = factory.LazyAttribute(lambda o: {"id": random.randint(1000, 9999)})

    @factory.post_generation
    def with_error(self, create, extracted, **kwargs):
        """Add error details to the response."""
        if extracted:
            self.status = "error"
            self.message = kwargs.get("message", "An error occurred")
            self.data = kwargs.get("data", {})
            self.error = kwargs.get("error", "Unknown error")
...
class ExpectedTransformedMessageFactory(factory.Factory):
    ...
    conversation_id = factory.Sequence(lambda n: f"conv_{n}")
    sender_id = factory.Sequence(lambda n: f"user_{n % 3}")
    sender_name = factory.Sequence(lambda n: f"User {n % 3}")
    content = factory.Faker("text", max_nb_chars=100)
    content_type = "text"
    timestamp = factory.LazyFunction(lambda: datetime.now().isoformat())
    edited = False
    ...
    class Params:
        """Parameters for creating different types of transformed messages."""
        ...
        poll = factory.Trait(
            content_type="poll",
            poll_question=factory.Faker("sentence"),
            poll_options=factory.LazyFunction(
                lambda: [faker.word() for _ in range(random.randint(2, 5))]
            )
        )


class ExpectedTransformedConversationFactory(factory.Factory):
    """Factory for generating expected transformed conversation data."""

    class Meta:
        model = dict

    conversation_id = factory.Sequence(lambda n: f"conv_{n}")
    display_name = factory.Sequence(lambda n: f"Conversation {n}")
    participants = factory.LazyFunction(
        lambda: [f"user_{i}" for i in range(random.randint(2, 5))]
    )
    ...
    @factory.post_generation
    def with_message_count(self, create, extracted, **kwargs):
        """Add a specific number of messages to the conversation."""
        if extracted:
            self.messages = [
                ExpectedTransformedMessageFactory(
                    conversation_id=self.conversation_id
