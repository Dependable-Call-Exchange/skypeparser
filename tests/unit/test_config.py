...
class TestConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_data = {
            ...
            "output": {"directory": "test-output", "overwrite": True},
    ...
    def test_load_config_from_file(self):
        ...
        with patch.dict(
            ...
            with patch(
                "os.path.exists", side_effect=lambda path: path == self.config_file
            ):
    ...
    def test_load_config_message_types(self):
        ...
        self.assertEqual(
            config["default_message_format"], "Test default format: {message_type}"
        )
        ...
        with patch.dict(
            ...
            with patch("os.path.exists", side_effect=mock_exists):
                config = load_config(
                    ...
                    message_types_file=self.message_types_file,
                )
                self.assertEqual(config["database"]["host"], "test-host")
                self.assertEqual(
                    config["message_types"]["Test/Type1"], "Test description 1"
                )
